...
Here, we're creating a new object with updated `count` value and returning it. But wouldn't it have been simpler to do something like this:
...
Instead, let's make use of a neat JavaScript feature which allows us to clone _all of the keys_ from one object into another. This way, in each of our `case`s we can say change this one thing and then just clone all of the current values for any other state that might exist. Here's what that might look like:
